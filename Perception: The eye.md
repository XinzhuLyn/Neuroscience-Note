...
|More photopigment, capture more light更多视色素以捕捉光|Less photopigment|更少的视色素
...
|Achromatic, due to only one kind of photopigment(Rhodopsin) 只含有一种视色素（视紫红素）因此仅有单色视觉|Chromatic多色视觉|
...
Darkness-->high cGMP concentration-->cGMP-gated channel(CNG) open-->Na+,Ca2+ influx, K+ efflux(not through CNG)-->depolarization

Light-->retinal conformationally changed(11-cis to all-trans configuration)-->no longer bind with opsin->opsin conformationally changed( change to metarhodopsin II)-->
transducin activated cGMP phosphodiesterase（PDE）, hydrolyzing cGMP-->low cGMP concentration-->cGMP gated channel close-->cut down Na+ influx，K+ efflux continue--> hyper polarization
